if(hp < global.maxbosshp/2) { fireRate = 60; }
switch(enemyState)
{
	case enemy.enter:
		y = smooth_approach(y, sprite_height*7/4, 0.06);
		if(y >= sprite_height*7/4-15) enemyState = enemy.active;
	break;
	case enemy.active:
		y = wave2(sprite_height*5/4, sprite_height*7/4, 1.6, 0.4, localtime);
		x = smooth_approach(x, dx, 0.05);
		localtime += 1/60;
		//x += hsp; y += vsp;
...
if(hp <= 0)
{
	audio_stop_sound(snd_hit2); sound(snd_explosion3); obj_game.shake = 12;
